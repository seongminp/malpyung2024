...
def create_top() -> Dict[str, "Component"]:
    ...
    with gr.Accordion(open=False) as advanced_tab:
        with gr.Row():
            ...
            quantization_method = gr.Dropdown(
                choices=["bitsandbytes", "hqq", "eetq"], value="bitsandbytes", scale=1
            )
            ...
            booster = gr.Radio(
                choices=["auto", "flashattn2", "unsloth", "liger_kernel"],
                value="auto",
                scale=3,
            )
    ...
    model_name.change(
        get_model_info, [model_name], [model_path, template, visual_inputs], queue=False
    ).then(
